...
# Condensed form of docs/TECHNICAL-SPEC-V1.md Appendix B. Qualitative anchors only —
# no numbers, no thresholds, no weights (C6). NOT empirically calibrated: statistical
...
_RATING_ANCHORS = (
    ...
    "later, deferred step. `rating` and `confidence` are independent: rate the level from "
    "the anchor, then set confidence separately (HIGH = evidence directly supports this "
    "level; MEDIUM = partial/indirect; LOW = mostly inference).\n"
    "ANTI-COMPRESSION: rate each dimension on its own merits. VERY_HIGH and HIGH are "
    "reachable and MUST be used when the evidence matches — do NOT default everything to "
    "LOW/MEDIUM. A dimension you genuinely cannot rate is LOW/LOW with a non-empty "
    "blocked_by (that is a correct outcome). 'Observed scale figure' below always means a "
    "number quoted from a source (hashtag post count, view/subscriber count, stated search "
    "trend) — never your own estimate.\n"
    "- signal_strength: LOW = one signal, or only inferred/hypothesis, or an adjacent "
    "theme. MEDIUM = 1-2 OBSERVED on the exact need, >=1 at MEDIUM confidence. HIGH = >=2 "
    "independent OBSERVED across >=2 sources/platforms, cluster-consistent. VERY_HIGH = "
    "HIGH plus an observed scale figure quoted from a source.\n"
    "- audience_potential: LOW = no size/reach evidence or a dead-end niche. MEDIUM = "
    "plausibly broad need, no data. HIGH = evidence points to a large/growing reachable "
    "audience. VERY_HIGH = observed mass audience, directly reachable on the target "
    "platform+language.\n"
    "- growth_momentum: LOW = existence only, no trajectory (OFTEN CORRECT — V1 has no "
    "time-series source; do not hide it). MEDIUM = a source calls it emerging/rising/"
    "trending, or a fresh dated EMERGING signal. HIGH = a source reports an observed "
    "increase. VERY_HIGH = multiple sources report rapid current growth with figures.\n"
    "- durability_opportunity_window: LOW = EPHEMERAL / a closed window. MEDIUM = EMERGING, "
    ...
    "VERY_HIGH = a specific angle with an obvious visual entry point, several treatments, "
    "and an existing creator behaviour to build on.\n"
    "- competitive_position (higher = more favorable): LOW = an incumbent dominates, the "
    "evidence is their traction, no angle, no assets. MEDIUM = contested but a credible "
    "entry. HIGH = competitors validate demand, none owns the specific angle/market, we "
    "hold an asset/cluster edge. VERY_HIGH = already positioned in this exact "
    "cluster+market+language with hero assets, angle unclaimed.\n"
    "- differentiation_potential: LOW = relabel/reframe, saturated generic category. "
    "MEDIUM = a plausible narrower angle, unproven. HIGH = a specific defensible angle "
    "(life-event, occasion, market/language gap, ritual) competitors don't serve. "
    "VERY_HIGH = unserved AND hard to copy.\n"
    "- asset_fit: LOW = nothing matches; best_* UNKNOWN/INFERRED; build from scratch. "
    "MEDIUM = partial (existing playlist OR classified/hero artist), new page needed, some "
    "INFERRED basis. HIGH = OBSERVED-basis playlist + >=1 OBSERVED-basis artist on "
    "cluster+market+language; only the page missing. VERY_HIGH = OBSERVED playlist + page "
    "+ artist already in the exact cluster+market+language.\n"
    "- business_outcome_potential: a SUMMARY, never an average of the 5 axes (C6). LOW = no "
    "credible path to consumption or reuse (a monitoring/risk signal). MEDIUM = one "
    "plausible engine (usually playlist growth via an existing playlist), scale unproven. "
    "HIGH = a clear asset-backed path on a primary engine (playlist growth / streaming "
    "royalty) or a strong observed UGC path. VERY_HIGH = asset-backed paths on more than "
    "one engine at once.\n"
    "- overall_confidence (LOW/MEDIUM/HIGH only): how much you actually KNOW the "
    "opportunity is real, correctly described and connectable — NOT a quality score, NOT "
    "raised by high dimension ratings. LOW = one signal or mostly inferred/hypothesis; key "
    "facts uncertain; asset path unknown. MEDIUM = >=2 corroborating OBSERVED signals fix "
    "the demand AND its market/language AND a known asset path exists, even if "
    "audience-size / growth / musical-DNA stay blocked_by — this is the NORMAL level for a "
    "well-evidenced V1 opportunity; do NOT dilute it to LOW just because volume data is "
