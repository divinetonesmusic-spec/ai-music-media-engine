"""Claude-in-the-loop plumbing for Page Blueprint (one call per opportunity).
...
(no ``output_config`` — same reasoning as Cluster Strategy/Evaluation: keeps the
grammar small and avoids the structured-output size limit). Tests never touch
the network.
"""
...
_MAX_TOKENS = 16000  # one PageBlueprint object — smaller than Cluster Strategy's, still headroom
                       # for default-effort thinking (mirrors Matching's 16000 budget)
...
def _extract_json_object(text: str) -> dict:
    ...
    if t[:1] != "{":
        start = t.find("{")
        if start == -1:
            raise ResponseRejected("page_blueprint: response carried no JSON object")
        depth = in_str = esc = 0
        end = None
    ...
    try:
        ...
        raise ResponseRejected(f"page_blueprint: model returned non-JSON: {e}") from e
    if not isinstance(payload, dict):
        raise ResponseRejected(
            f"page_blueprint: response is a {type(payload).__name__}, not a JSON object"
        )
    return payload


class AnthropicPageBlueprintClient(StageClient):
    ...
    def _build(self):
        ...
        try:
            ...
            raise StageError("page_blueprint needs the 'anthropic' package") from e
    ...
    def complete(self, *, stage, key, prompt, schema, model) -> dict:
        ...
        try:
            ...
            raise StageError(
                f"page_blueprint: the model call did not return within "
                f"~{self._READ_TIMEOUT:.0f}s"
            ) from e
        except anthropic.APIError as e:
            raise StageError(_redact(f"page_blueprint API call failed: {e}")) from e
        ...
        if not text:
            ...
            raise ResponseRejected(
                f"page_blueprint: no text block (stop_reason={stop!r})"
            )
...
def select_client(
    *,
    ...
    return AnthropicPageBlueprintClient(), "live"
